...
export async function POST(request) {
    ...
    if (success) {
        const { Title, Description, Amount, Type, Date, TimeStamp, AccType } = await request.json();
        console.log("🚀 ~ POST ~ Title, Description, Amount, Type, Date, TimeStamp, AccType :", Title, Description, Amount, Type, Date, TimeStamp, AccType)
        if (!Title) {
            ...
            return NextResponse.json({ message: "Please Enter Description", status: false }, { status: 400 });
        } else if (!Date) {
            ...
            return NextResponse.json({ message: "Please Enter AccType", status: false }, { status: 400 });
        } else {
            await connectMongoDB();
            await Income.create({ Title, Amount, Type, Date, TimeStamp, User_id: user?.userId, Description, AccType });
